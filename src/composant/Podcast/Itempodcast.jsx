...
function Itempodcast() {
  ...
  return (
    <div className="py-10 bg-rose-50 flex flex-col items-center">
      <div
        className="flex justify-center items-center mt-8 cursor-pointer w-fit"
        onClick={(e) => setliscatart(!liscatart)}
      >
        <h3 className="text-xl font-semibold text-marron-200 2xl:text-4xl">
          {catpodcast
            .filter((cat) => cat.id === catpodchoix)
            .map((e) => {
              return e.titre;
            })}
        </h3>{" "}
        <ChevronDownIcon className="h-5 ml-4 2xl:h-10 text-blue-200" />
      </div>
      <ul
        className={`fixed ${
      ...
      >
        ...
        {catpodcast.map((cat) => {
          return (
            <li
              key={cat.id}
              className=" cursor-pointer text-2xl 2xl:text-4xl font-semibold mb-6"
              onClick={(e) => {
                setcatpodchoix(cat.id);
