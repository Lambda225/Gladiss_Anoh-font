...
function Slidearticle() {
  ...
  return (
    <div className='my-20'>
        ...
        <div className='flex justify-center items-center mt-8 cursor-pointer ' onClick={e => setliscatart(!liscatart)}><h3 className='text-xl font-semibold text-marron-200 2xl:text-3xl'>{catarticle.filter(cat => cat.id == catartchoix).map(e => {return e.titre})}</h3> <ChevronDownIcon className='h-5 ml-4 text-blue-200' /></div>
        <ul className={`fixed ${liscatart ? 'flex' : 'hidden'} z-50 top-0 left-0 w-full h-screen bg-white opacity-90 flex-col justify-center items-center`} onClick={e => setliscatart(!liscatart)}>
          ...
          {
            catarticle.map(cat => {
              return <li key={cat.id} className=' cursor-pointer text-2xl font-semibold mb-6 2xl:text-4xl' onClick={e => {setcatartchoix(cat.id);setliscatart(!liscatart)}}>{cat.titre}</li>
            })
          }
        </ul>
        <div>
            <div className="px-10">
                <Carousel responsive={responsive} autoPlay={false} infinite={true} className="py-10 z-0">
                  {
                    articles.filter((cat) => cat.idcategorie == catartchoix).slice(0,8).map(art => {
                      return <div key={art.id} className="flex justify-center"><Carteart opencatre={opencatre} article={art} itemart={itemart} setitemart={setitemart} setopencatre={setopencatre} /></div>
                    })
                  }
                </Carousel>
            </div>
